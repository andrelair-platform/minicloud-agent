...
Deployment config lives in the GitOps repo: **minicloud-gitops/services/minicloud-agent/helm/ (GAP wrapper chart + kargo/)**.
...
- **Deploy config is NOT in this repo** — image tag / replicas / ingress / secrets live in the GitOps
  repo (minicloud-gitops/services/minicloud-agent/helm/ (GAP wrapper chart + kargo/)). Changing how it *runs* = a PR there, not here.
